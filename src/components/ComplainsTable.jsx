...
import { BASE_URL, COMPLAINS } from '../api/Api';
...
function ComplainsTable() {
    ...
    useEffect(() => {
        ...
        Axios.get(`${COMPLAINS}`,)
            .then(function (response) {
                ...
                setComplains(response.data.reverse());
                setLoading(false);
            })
    ...
    async function handleDeleteConfirm() {
        ...
        try {
            const response = await axios.delete(`${BASE_URL}${COMPLAINS}/${rowToDelete}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            })
            const updatedRow = complains.filter((row) => row.id !== rowToDelete);
            // console.log(response);
            setComplains(updatedRow);
            handleDeleteClose();
            setLoading(false);
    ...
    const renderedRows = useMemo(() => {
        return complains.map((row, index) => {
        ...
        <div dir={locale === "en" ? "ltr" : "rtl"} className={[locale === "en" ? "ltr" : "rtl", "categories"].join(" ")}>
            <Stack direction='row' sx={{ justifyContent: 'space-between', alignItems: 'center', my: 2 }}>
                ...
                <div className="stack-group">
                    <a className="stack-export-link" href="https://construction.site-pocket.com/api/export-complaints" target="_blank" rel="noopener noreferrer">
