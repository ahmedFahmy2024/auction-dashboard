...
import { BASE_URL, DIGITAL_TRANFORMATION } from '../api/Api';
...
function DigitalTransformationTable() {
    ...
    //  ====== get all digital-transformation ========
    useEffect(() => {
        setLoading(true);
        Axios.get(`${DIGITAL_TRANFORMATION}`,)
            .then(function (response) {
                // console.log(response.data);
                setTransformations(response.data.reverse());
                setLoading(false);
            })
    ...
    function handleEditClick(id) {
        navigate(`/DigitalTransformation/${id}/Edit`);
    }
    //  ================ edit function ================
    ...
    async function handleDeleteConfirm() {
        ...
        try {
            const response = await axios.delete(`${BASE_URL}${DIGITAL_TRANFORMATION}/${rowToDelete}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            })
            const updatedRow = transformations.filter((row) => row.id !== rowToDelete);
            // console.log(response);
            setTransformations(updatedRow);
            handleDeleteClose();
            setLoading(false);
    ...
    const columns = useMemo(
        () => [
            ...
            {
                accessorKey: 'company_specialization',
    ...
    const renderedRows = useMemo(() => {
        return transformations.map((row, index) => {
        ...
        <div dir={locale === "en" ? "ltr" : "rtl"} className={[locale === "en" ? "ltr" : "rtl", "categories"].join(" ")}>
            <Stack direction='row' sx={{ justifyContent: 'space-between', alignItems: 'center', my: 2 }}>
                ...
                <div className="stack-group">
                    <a className="stack-export-link" href="https://construction.site-pocket.com/api/export-digital-transformations" target="_blank" rel="noopener noreferrer">
...
export default DigitalTransformationTable