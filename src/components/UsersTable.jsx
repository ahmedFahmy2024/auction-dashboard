...
function UsersTable() {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        accessorFn: row => (row.current_subscription ? (row.current_subscription.remaining_days < 0 ? '-' : row.current_subscription.package_name) : '-'),
      ...
      {
        accessorFn: row => (row.current_subscription ? (row.current_subscription.remaining_days < 0 ? '-' : row.current_subscription.end_date) : '-'),
        header: (t('End Date')),
        size: 150,
        muiTableHeadCellProps: {
          align: 'center',
      ...
      {
        accessorFn: row => (row.current_subscription ? (row.current_subscription.remaining_days < 0 ? '-' : row.current_subscription.remaining_days) : '-'),
    ...
    <div dir={locale === "en" ? "ltr" : "rtl"} className={[locale === "en" ? "ltr" : "rtl", "userstable"].join(" ")}>
      <Stack direction='row' sx={{ justifyContent: 'space-between', alignItems: 'center', my: 2 }}>
        ...
        <div className="stack-group">
          ...
          <a className="stack-export-link" href="https://construction.site-pocket.com/api/export-users" target="_blank" rel="noopener noreferrer">
