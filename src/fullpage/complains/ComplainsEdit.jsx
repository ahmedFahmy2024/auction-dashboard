...
import ComplainsChat from "./ComplainsChat";
...
function ComplainsEdit() {
    ...
    const fetchComplain = async () => {
        try {
            const response = await Axios.get(`${COMPLAINS}/${id}`);
    ...
    const handleStatusChange = async (e) => {
        ...
        try {
            setLoading(true);
            await Axios.put(`${COMPLAINS}/${id}`, { status: newStatus });
                ...
                <Grid container spacing={2}>
                    <Grid xs={12} md={8}>
                        <div className="chat-area">
                            {complains && <ComplainsChat complains={complains} setUpdateTrigger={setUpdateTrigger} />}
                        </div>
                    </Grid>
                    <Grid xs={12} md={4}>
