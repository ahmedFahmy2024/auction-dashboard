...
function SupplierRegistrationEdit() {
    ...
    const fetchComplain = async () => {
        try {
            const response = await Axios.get(`${SUPPLIER_REGISTRATION}/${id}`);
    ...
    const handleStatusChange = async (e) => {
        ...
        try {
            setLoading(true);
            await Axios.put(`${SUPPLIER_REGISTRATION}/${id}`, { status: newStatus });
                ...
                <Grid container spacing={2}>
                    <Grid xs={12} md={8}>
                        <div className="chat-area">
                            {complains && <RegistrationChat complains={complains} setUpdateTrigger={setUpdateTrigger} />}
                        </div>
                    </Grid>
                    <Grid xs={12} md={4}>
